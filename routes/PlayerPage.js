...
router.get('/api/player/Data', async (req, res) => {
    ...
    try {
        const query = 'SELECT giocatori.id_giocatore,\n' +
            ...
            '                     SUM(CASE WHEN prestazioni.titolare THEN 1 ELSE 0 END) AS total_titolare_true,\n' +
            '                     SUM(CASE WHEN prestazioni.titolare THEN 0 ELSE 1 END) AS total_titolare_false\n' +
            '              FROM giocatori\n' +
            '              JOIN prestazioni ON giocatori.id_giocatore = prestazioni.id_giocatore\n' +
            '              JOIN squadre ON giocatori.id_squadra = squadre.id_squadra\n' +
